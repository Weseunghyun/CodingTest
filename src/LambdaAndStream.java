...
public class LambdaAndStream {
    public static void main(String[] args) {
       ...
       // parkingLot.addAll(parkingCarWithTicket(carsWantToPark));
        parkingLot.addAll(parkingCar(carsWantToPark, Car::hasTicket));

        //parkingLot.addAll(parkingCarWithMoney(carsWantToPark));
        parkingLot.addAll(parkingCar(carsWantToPark, Car::noTicketButMoney));

        weekendParkingLot.addAll(parkingCar(carsWantToPark, (car)-> car.hasParkingTicket() && car.getParkingMoney()>1000));
...
//    }
    ...
    public static List<Car> parkingCar(List<Car> carsWantToPark, Predicate<Car> function) {
...
//티켓이 있는지 돈이 있는지 전부 boolean 타입으로 변환하므로 함수형 인터페이스 predicate를 만들어
//test를 진행할거임.
